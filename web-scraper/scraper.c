...
void init_scraper_pool(int thread_count) {
    LOG_INFO("Creating thread pool with %d threads and queue size %d", thread_count, QUEUE_SIZE);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--threads") == 0 && i + 1 < argc) {
            ...
            printf("  --stats-interval N Statistics reporting interval in seconds (default: %d)\n", STATS_INTERVAL);
    ...
    if (init_redis(redis_host, redis_port) != 0) {
        LOG_ERROR("Failed to initialize Redis. Please check the error messages above for instructions.");
        curl_global_cleanup();
        return EXIT_FAILURE;
    }
    ...
    if (pthread_create(&monitor_tid, NULL, monitor_thread, (void *)&stats_interval) != 0) {
